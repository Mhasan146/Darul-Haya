...
export async function sendContactEmail({ name, email, message }) {
  ...
  return resend.emails.send({
    ...
    html: `<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;color:#1E4A35">
      <h2 style="color:#21804E">New message from the website</h2>
      <p><strong>Name:</strong> ${escapeHtml(name)}</p>
      <p><strong>Email:</strong> ${escapeHtml(email)}</p>
      <p style="margin-top:16px"><strong>Message:</strong></p>
      <p style="white-space:pre-line;background:#EEF8F0;border-radius:8px;padding:16px">${escapeHtml(message)}</p>
    </div>`,
  })
}
...
export async function sendOpenHouseEmail({ name, email, phone, grade, session }) {
  ...
  return resend.emails.send({
    ...
    html: `<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;color:#1E4A35">
      <h2 style="color:#21804E">New open house RSVP</h2>
      <p><strong>Session:</strong> ${escapeHtml(session || 'Not specified')}</p>
      <p><strong>Name:</strong> ${escapeHtml(name)}</p>
      <p><strong>Email:</strong> ${escapeHtml(email)}</p>
      <p><strong>Phone / WhatsApp:</strong> ${escapeHtml(phone || 'Not given')}</p>
      <p><strong>Child&rsquo;s grade:</strong> ${escapeHtml(grade || 'Not given')}</p>
      <p style="margin-top:16px;color:#47584d">Send this family the details for their chosen session.</p>
    </div>`,
  })
}
          ...
          <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.06);">
            ...
            <tr>
              <td style="padding:36px 32px;">
                ...
                <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#47584d;">
                ...
                <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#47584d;">
            ...
            <tr>
              <td style="padding:20px 32px;border-top:1px solid #eeeeee;text-align:center;">
                <p style="margin:0;font-size:12px;color:#93a398;">Faith &bull; Knowledge &bull; Character</p>
                <p style="margin:6px 0 0;font-size:12px;color:#93a398;">&copy; ${year} Darul Haya. All rights reserved.</p>
              </td>
            </tr>
          </table>
        </td>
