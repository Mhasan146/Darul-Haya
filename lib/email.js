 ...
 */
export async function sendOpenHouseEmail({ name, email, phone, grade }) {
  const resend = new Resend(process.env.RESEND_API_KEY)
  const to = process.env.CONTACT_EMAIL || DEFAULT_FROM
  return resend.emails.send({
    from: `Darul Haya Website <${DEFAULT_FROM}>`,
    to,
    replyTo: email,
    subject: `New open house RSVP from ${name}`,
    html: `<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;color:#134E4A">
      <h2 style="color:#0D9488">New online open house RSVP</h2>
      <p><strong>Name:</strong> ${escapeHtml(name)}</p>
      <p><strong>Email:</strong> ${escapeHtml(email)}</p>
      <p><strong>Phone / WhatsApp:</strong> ${escapeHtml(phone || '—')}</p>
      <p><strong>Child&rsquo;s grade:</strong> ${escapeHtml(grade || '—')}</p>
      <p style="margin-top:16px;color:#475a57">Add this family to the WhatsApp open house group / send them the join details.</p>
    </div>`,
  })
}
