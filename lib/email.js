...
export async function sendContactEmail({ name, email, message }) {
  ...
  return resend.emails.send({
    ...
    html: `<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;color:#16402B">
      <h2 style="color:#1E7A48">New message from the website</h2>
      <p><strong>Name:</strong> ${escapeHtml(name)}</p>
      <p><strong>Email:</strong> ${escapeHtml(email)}</p>
      <p style="margin-top:16px"><strong>Message:</strong></p>
      <p style="white-space:pre-line;background:#F1FAF4;border-radius:8px;padding:16px">${escapeHtml(message)}</p>
    </div>`,
  })
}
...
export async function sendOpenHouseEmail({ name, email, phone, grade, session }) {
  ...
  return resend.emails.send({
    ...
    html: `<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;color:#16402B">
      <h2 style="color:#1E7A48">New open house RSVP</h2>
